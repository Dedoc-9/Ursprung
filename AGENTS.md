...
The one-line summary of the renderer thesis: **arbitrary boundaries require deterministic
...
Two invariants the kernel adds, enforced *by type* in the Rust CORE port (`experiments/reality_kernel/core_rs/`, verified on real silicon,
`cargo test` green — semantic preservation + adversarial concurrency): **a dropped observation is allowed; a
...
## weltwerk — the verification kernel and its applications
...
(value-parity on frob/σ_max/ρ/step + decision-parity), and **`--schema kappa` ingests it from a dump**:
**Schema C** is a fixed 160-byte κ-block (`frame` + 16 κ row-major + `λ,dt,σ`, n=4), certified in O(1) memory
...
/ simulation codebase, independent of Ursprung's own modules. **Usable as-is, nothing to run.**
...
**3. As a measurement / audit substrate, and a representation *classifier*.** Point
...
**4. As a causal-attribution discipline for residuals / anomalies / ghosts.**
...
The conceptual arc is **complete** (a deterministic stdlib check suite — `python tests/test_ursprung.py` is
the falsifier; counts drift, so re-run it rather than trusting a number here), the empirical phase **ran**, and
it has **crossed onto real silicon.** The conceptual genealogy: M1 foundation + invariant harness → M2 the five laws → M3/3.1
...
benchmark, verified on hardware across the spatial and temporal benchmark envelope** (`experiments/bench_gpu_real`,
...
`docs/BOUNDARY_MAP.md`. The durable artifact is the apparatus that could measure the boundary
in either direction.
...
kernel (many actors, one region — the concurrency test the live kernel has not faced), plus the standing seams —
