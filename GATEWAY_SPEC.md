...
- **Build status.** Ported to Rust and `cargo test`-green in the `ursprung` crate, each differential-tested
  ...
  byte-for-byte matched to the Python — **not** zero-copy (it decodes into typed fields). Still OPEN: binding
  the gate to live test execution (`static-check ≠ live-execution`), the Rust↔Python ledger drift
  (`mirror ≠ source`), and the single-binary assembly itself. `parts-ported ≠ monolith-built`.
