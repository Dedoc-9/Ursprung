...
| `verified_improvement_theorem.py` | **the theorem** — verified self-improvement is a Bienaymé–Galton–Watson branching process: verified mean `m ≤ 1` ⇒ a.s. extinction (recursion impossible); `m > 1` ⇒ survives w.p. `1−q`; `m̂ > 1 ≥ m` is the runaway's signature. Monte-Carlo verified against the analytic extinction fixed point across sub/critical/supercritical regimes | IMPLEMENTED | 7/7 |
