...
Verified by `--selftest` (GPU-free, 5/5): observer mutation can't reach the gated state; only a Grounded action
commits; an ungrounded commit is refused; out-of-band tampering fails closed; replay is deterministic.
...
class AirGap:
    ...
    def commit(self, grounded, mutate_fn):
        """Only a Grounded action (Phase 9, duck-typed `.get()`) may mutate the gated state; re-seal after."""
        if not hasattr(grounded, "get"):
            raise PermissionError("commit requires a Grounded action (Phase 9) — observation != authority")
        mutate_fn(self._state, grounded.get())
        self._hash = state_hash(self._state)
        return self._hash
...
class _G:                                # minimal Grounded stub (Phase 9 provides the real Grounded[T])
...
def selftest() -> int:
    ...
    print(f"[selftest] only a Grounded action commits + re-seals                  : {ok_commit}")
    ...
    ok = ok_nowrite and ok_commit and ok_ungrounded and ok_tamper and ok_replay
    print(f"[selftest] {'PASS 5/5 - air-gap layer valid (observe-only + tamper-evident + replayable)' if ok else 'FAIL'}")
